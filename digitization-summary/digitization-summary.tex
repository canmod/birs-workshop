...
% Much of the data came as scans of paper documents. These data were manually entered into the computer as Excel spreadsheets that replicate the format of the original scans. Optical character recognition (OCR) was attempted to automate this process, but we found that the accuracy was not sufficient. For example, the incidence data before 1956 come as handwritten documents including numbers that are not easily legible. But even typewritten documents after 1956 are not reliably intepreted by our OCR tools.
%
% Ideally we would like users of the archive to be able to easily generate long time series of particular diseases in particular locations. Steve discussed challenges in pulling various data sources together to provide such a user experience, including the following.
%
% \begin{itemize}
%   \item Disease and place names change over time
%   \item Disease codes (ICD) change over time
%   \item Hierarchical data (e.g. diseases within families of diseases), with hierarchies that change over time
%   \item Inconsistent time periods (e.g. weekly shifts to monthly)
%   \item Age group definitions change over time
% \end{itemize}
%
% Solutions to these issues were discussed briefly.
